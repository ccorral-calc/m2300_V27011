...
void do_loop_test(struct ohci_link_regs_t *link_ptr);
...
int initialize_ohci(struct ohci_link_regs_t *ohci_reg_ptr)
{
  ...
  if(BusSpeed == 2)
    temp = (temp & 0xFFFFFFF8) | 0x2;  //force link speed to S400. bdu100307
 ...
 *
 ****************************************************/
int get_node_map(PHY_NODE *node,int *isRemote)
{
  int node_count = 0;
  int gen;
  int size;
  ...
  for(i=0; i<size; i++)
	...
	{
	  if(node[i].p0 == 1 && node[i].p1 == 2 && node[i].p2 == 1) //p0 has parent, p1, p2 N/C, cartridge
          {
	    node[i].type = Calculex_Cartridge;
            *isRemote = 0;
          }
	  else if(node[i].p0 == 2 && node[i].p1 == 1 && node[i].p2 == 1) //p0 has parent, p1, p2 N/C, cartridge
          {
	    node[i].type = Calculex_Cartridge;
            *isRemote = 0;
          }
	  ...
	  else if(node[i].p0 == 3 && node[i].p1 == 1 && node[i].p2 == 3) //p0, p1 N/C, p2 has child, controller
          { 
	    node[i].type = Calculex_Controller;
            *isRemote = 1;
          }
	  else if(node[i].p0 == 1 && node[i].p1 == 3 && node[i].p2 == 3) //p0, p1 N/C, p2 has child, controller
          { 
	    node[i].type = Calculex_Controller;
            *isRemote = 1;
          }
      ...
      if(debug)
      {      
          printf("node %d, self ID = 0x%08lx, p0=%d, p1=%d, p2=%d, link=%d,Remote %d\r\n", i, temp, node[i].p0, node[i].p1, node[i].p2, node[i].link_active,*isRemote);
      }
    }
...
int initialize_scsi_orb(int node_id)
{
  ...
  *ptmp32 = OHCI_SWAP_FOUR(*ptmp32);

#if 1
  //set address for each page once page table bit is set

  for(i=0; i<32; i++)
  {
      WRITE32(0x80040500 + i*8, OHCI_SWAP_FOUR(0x80000000));

      if(i %2)
      {
          WRITE32(0x80040504 + i*8, OHCI_SWAP_FOUR(0x00058000));
      }
      else
      {
          WRITE32(0x80040504 + i*8, OHCI_SWAP_FOUR(0x00050000));
      }
  }
#endif
  ...
  WRITE32(DRP_RESERVED_SPACE_A, OHCI_SWAP_FOUR(0xFFC00000 | (node_id << 16)));
  WRITE32(DRP_RESERVED_SPACE_A+4, OHCI_SWAP_FOUR(DPR_BASE_ADDRESS + DPR_AT_SCSI_ORB));
  WRITE32(DRP_RESERVED_SPACE_B, OHCI_SWAP_FOUR(0xFFC00000 | (node_id << 16)));
  WRITE32(DRP_RESERVED_SPACE_B+4, OHCI_SWAP_FOUR(DPR_BASE_ADDRESS + DPR_AT_SCSI_ORB));
 ...
 *****************************************************************************/
int send_login_orb(struct ohci_link_regs_t *link_ptr, int phy_id, int target_id)
{
  struct dual_port_RAM_t *dpr_ptr;
  UINT32 temp;
  int i;
  ...
  WRITE32(DRP_RESERVED_SPACE_A, OHCI_SWAP_FOUR(0xFFC00000 | (phy_id << 16)));
  WRITE32(DRP_RESERVED_SPACE_A + 4, OHCI_SWAP_FOUR((DPR_BASE_ADDRESS + DPR_AT_LOGIN_ORB)));
  ...
  //prepare the data address
  WRITE32(DRP_RESERVED_SPACE_B, OHCI_SWAP_FOUR(0xFFC00000 | (phy_id << 16)));
  WRITE32(DRP_RESERVED_SPACE_B + 4, OHCI_SWAP_FOUR((DPR_BASE_ADDRESS + DPR_AT_LOGIN_ORB)));
...
int  read_blocks(struct ohci_link_regs_t *link_ptr, int phy_id, CONTEXT context, UINT32 block, int num_blocks )
{
  ...
  switch(context)
    ...
    case context_a:
      ...
      //prepare the data address
      WRITE32(DRP_RESERVED_SPACE2, OHCI_SWAP_FOUR(0xFFC00000 | (phy_id << 16)));
      WRITE32(DRP_RESERVED_SPACE2 + 4, OHCI_SWAP_FOUR((DPR_BASE_ADDRESS + DPR_AT_SCSI_ORB)));  //0x41200
...
void do_loop_test(struct ohci_link_regs_t *link_ptr) 
{
  ...
  if(BusSpeed == 2)
  {
      ...
      //code from TI. bdu102706
      num_ports = read_phy_reg(link_ptr, 2) & 0xF;
      for(i=0; i<num_ports; i++)
        {
          write_phy_reg(link_ptr, 7, i);

          temp = read_phy_reg(link_ptr, 8);
          temp |= 0x1;  //disable the port
          write_phy_reg(link_ptr, 8, temp);

          /* change speed to S400 */
          write_phy_reg(link_ptr, 10, (2<<4));
          write_phy_reg(link_ptr, 13, 0x1);  //set the hard disable bit

          temp &= ~0x1;  //enable the port
          write_phy_reg(link_ptr, 8, temp);
        }
